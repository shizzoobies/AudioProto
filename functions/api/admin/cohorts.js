...
export async function onRequestGet({ env }) {
  ...
  try {
    ...
    return await listCohorts(env);
  } catch (e) {
    return jsonError('list_failed', 500, String(e?.message || e));
  }
}

async function listCohorts(env) {
...
async function assignManagers(body, request, env) {
  const cohortId = typeof body?.cohort_id === 'string' ? body.cohort_id.trim() : '';
  if (!cohortId) return jsonError('cohort_id_required', 400);
  const cohort = await env.DB.prepare(`SELECT id FROM cohorts WHERE id = ?`).bind(cohortId).first();
  ...
  for (const m of members) {
    ...
    await env.DB
      ...
      .bind(inviteId, tokenHash, token, m.email, m.name, now, createdBy)
      .run();
    await env.DB
