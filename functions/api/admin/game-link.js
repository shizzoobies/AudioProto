// Admin endpoint for the single token-gated "Designing Growth" game link. Backed
// by the existing invites table (no migration): the game link is the invites row
// whose recipient_email is the GAME_RECIPIENT_EMAIL sentinel. It carries no
// scenario assignments — it only releases the static /designing-growth page.
// There is at most one active game link. Middleware enforces cs_admin on every
// method. Mirrors functions/api/admin/charts.js.
//
// GET    - status: { active, created_at, last_click_at }
// POST   - create-or-refresh, rotate the token, return { url } (token shown once)
// DELETE - revoke the game link (idempotent)

import { sha256Hex, randomId, randomToken, GAME_RECIPIENT_EMAIL } from '../../../shared/auth.js';

export async function onRequestGet({ env }) {
  if (!env.DB) return jsonError('db_not_configured', 500);
  try {
    const row = await findGameLink(env);
    return json({
...
async function createOrRefresh(request, env) {
  ...
  const existing = await findGameLink(env);
  if (existing) {
    ...
    await env.DB
      .prepare(`INSERT INTO invites
                ...
                VALUES (?, ?, ?, ?, ?, NULL)`)
      .bind(randomId(), tokenHash, GAME_RECIPIENT_EMAIL, 'Designing Growth', now)
      .run();
  }

  // Token is shown once; only its hash is stored.
  const url = `${origin}/game-pass/${token}`;
  return json({ url }, 201);
}
