...
import { sha256Hex, randomId, randomToken, COACHING_ADMIN_RECIPIENT_EMAIL } from '../../../shared/auth.js';

export async function onRequestGet({ env }) {
  if (!env.DB) return jsonError('db_not_configured', 500);
  try {
    const row = await findCoachingAccessLink(env);
    return json({
...
export async function onRequestDelete({ env }) {
  ...
  try {
    ...
    const res = await env.DB
      ...
      .bind(now, COACHING_ADMIN_RECIPIENT_EMAIL)
      .run();
...
async function createOrRefresh(request, env) {
  ...
  const existing = await findCoachingAccessLink(env);
  if (existing) {
    await env.DB
    ...
    await env.DB
      .prepare(`INSERT INTO invites
                ...
                VALUES (?, ?, ?, ?, ?, NULL)`)
      .bind(randomId(), tokenHash, COACHING_ADMIN_RECIPIENT_EMAIL, 'Scenarios editor', now)
      .run();
  }
