// Public landing for the open "Back-to-back demo reel" link. Admins generate
// URLs like /reel/<token>; this route validates the token against the invites
// table (sha256 hash lookup), confirms the row is the reel sentinel for safety,
...
// redirects into /app. Because the cookie is a normal cs_me cookie, the existing
// getInviteScope + voice-agent scope check already grant access to the five reel
// scenarios assigned to this invite - no extra wiring needed. The client reads
// is_reel off /api/me/status and enters the reel experience.
//
// This is NOT under /api, so the API middleware does not gate it. It is the
// public, no-password entry point for the reel. Mirrors functions/demo/[token].js.

import { signToken, sha256Hex, REEL_RECIPIENT_EMAIL } from '../../shared/auth.js';
...
export async function onRequest({ request, env, params }) {
  ...
  // Safety: only the reel sentinel row may be entered through /reel/*. A normal
  // recipient invite token (or the demo sentinel) must not be honored here.
  if (row.recipient_email !== REEL_RECIPIENT_EMAIL) return errorPage();
  ...
  // request fails the instant the admin revokes or regenerates the reel invite.
