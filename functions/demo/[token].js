// Public landing for the open demo link. Admins generate URLs like
// /demo/<token>; this route validates the token against the invites table
// (sha256 hash lookup), confirms the row is the demo sentinel for safety,
...
// redirects into /app. Because the cookie is a normal cs_me cookie, the
// existing getInviteScope + chat/tts/coach scenario lock already grant access
// to the two demo scenarios assigned to this invite - no extra wiring needed.
//
// This is NOT under /api, so the API middleware does not gate it. It is the
// public, no-password entry point for the demo.

import { signToken, sha256Hex, DEMO_RECIPIENT_EMAIL } from '../../shared/auth.js';
...
export async function onRequest({ request, env, params }) {
  ...
  // Safety: only the demo sentinel row may be entered through /demo/*. A normal
  // recipient invite token must not be honored here.
  if (row.recipient_email !== DEMO_RECIPIENT_EMAIL) return errorPage();
  ...
  // request fails the instant the admin revokes or regenerates the demo invite.
