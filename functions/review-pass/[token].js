// Public landing for the token-gated review-editor link. Admins generate URLs
// like /review-pass/<token>; this validates the token against the invites table,
// confirms it's the review sentinel row, issues a cs_review cookie scoped to that
// invite_id (carrying the current token_hash), bumps the click counter, and
// redirects to /admin — where the SPA detects the reviewer scope and renders the
// rubric-only editor.
//
...
export async function onRequest({ request, env, params }) {
  ...
  if (row.recipient_email !== REVIEW_RECIPIENT_EMAIL) return errorPage();
  ...
  const cookie = await signToken(
    { scope: 'review', invite_id: row.id, h: tokenHash, iat: now, exp },
    env.SESSION_SECRET
  );
  ...
  headers.append('Set-Cookie', buildCookie('cs_review', cookie, COOKIE_TTL_SECONDS, isHttps));
